 ...
 * a real ERC-20 transfer on Sepolia / Hedera Testnet — visible on Etherscan.
 *
...
export default function CoNNFTMarketplace() {
  ...
  const handleBuy = async (nft: typeof NFTS[0]) => {
    ...
    try {
      ...
      setTxUrl(`https://sepolia.etherscan.io/tx/${transferTx}`);
    ...
    <main style={{ paddingBottom: 80 }}>
      ...
      <div style={{ padding: '20px 16px 0', display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', margin: 0 }}>
            Conservation NFTs · Pay with yBOB · Sepolia / Hedera Testnet
          </p>
        </div>
      ...
      {!YBOB_ADDR && (
        <div style={{ margin: '12px 16px 0', background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.3)', borderRadius: 12, padding: '10px 14px', fontSize: 11, color: 'rgba(255,255,255,0.7)' }}>
          <strong style={{ color: '#F97316' }}>yBOB not deployed.</strong> Run <code style={{ color: '#fbbf24' }}>deploy.ts --network sepolia</code> first.
        </div>
      )}
